...
/**
 * "In the last 14 days" strip — same shape as the coach-side strip but
 * anchored on a rolling window since consultants don't own recaps.
 * Quiet when nothing is new.
 */
export function ConsultantSinceStrip({ stats }: { stats: ConsultantSinceStats }) {
  if (!hasAnyNewConsultantActivity(stats)) {
    return (
      <div className="rounded-lg border border-dashed border-neutral-300 bg-white px-4 py-2 text-xs text-neutral-500">
        ...
        quiet — no new activity from this learner.
      </div>
    );
  }
  ...
  if (stats.newPreSessionNotes > 0) {
    chips.push({
      label: `${stats.newPreSessionNotes} new prep`,
      tone: "pink",
    });
  }
