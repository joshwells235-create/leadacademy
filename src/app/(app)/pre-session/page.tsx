    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      {notes && notes.length > 0 && (
        <div className="mt-8 space-y-4">
          ...
          {notes.map((n) => (
            <div key={n.id} className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm text-sm">
              <div className="text-xs text-neutral-500 mb-2">
                {n.session_date ? `Session: ${n.session_date}` : new Date(n.created_at).toLocaleDateString()}
              </div>
              <p className="text-neutral-900 font-medium">Want to discuss:</p>
              <p className="text-neutral-700 mb-2">{n.want_to_discuss}</p>
              {n.whats_been_hard && (<><p className="text-neutral-900 font-medium">What's been hard:</p><p className="text-neutral-700 mb-2">{n.whats_been_hard}</p></>)}
              {n.whats_going_well && (<><p className="text-neutral-900 font-medium">What's going well:</p><p className="text-neutral-700">{n.whats_going_well}</p></>)}
            </div>
          ))}
        </div>
      )}
