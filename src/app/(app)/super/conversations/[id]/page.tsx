...
export default async function ConversationViewPage({ params }: Props) {
  ...
  const { data: convo } = await supabase
    ...
    .select("id, mode, user_id, org_id, profiles:user_id(display_name), organizations:org_id(name)")
    .eq("id", id)
    .maybeSingle();
  if (!convo) notFound();
      ...
      <div className="space-y-3">
        {(messages ?? []).map((m) => {
          ...
          return (
            <div
              key={m.id}
              className={`rounded-lg p-4 ${m.role === "user" ? "bg-brand-blue/10 border border-brand-blue/20" : m.role === "assistant" ? "bg-white border border-neutral-200 shadow-sm" : "bg-neutral-100 border border-neutral-200"}`}
            >
              <div className="flex items-center justify-between mb-2">
                <span
                  className={`text-xs font-medium ${m.role === "user" ? "text-brand-blue" : m.role === "assistant" ? "text-brand-navy" : "text-neutral-500"}`}
                >
                  {m.role === "user"
                    ? "Learner"
                    : m.role === "assistant"
