...
### Coach
- `/coach/dashboard` — searchable / filterable / sortable learner grid, per-card "what's new since your last recap" chips (including D4 "N flagged questions" chip), sprint vitality, coaching-since date
- `/coach/learners/[id]` — top "since last recap" strip, prev/next nav (←/→ keyboard), sprint vitality on goals, thought-partner activity panel (read-only), AI-drafted session recap button, full action-item / notes / recap UI, `FlaggedQuestions` panel (D4) with waiting-first ordering + inline reply editor
